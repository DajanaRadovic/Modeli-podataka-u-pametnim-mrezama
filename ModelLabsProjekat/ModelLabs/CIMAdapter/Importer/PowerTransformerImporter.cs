...
namespace FTN.ESI.SIMES.CIM.CIMAdapter.Importer
{
	...
	public class PowerTransformerImporter
	{
		...
		{
			SortedDictionary<string, object> cimConductingEqs = concreteModel.GetAllObjectsOfType("FTN.ConductingEquipment");
			if (cimConductingEqs != null)
			{
				foreach (KeyValuePair<string, object> cimConductingEqPair in cimConductingEqs)
				{
					FTN.ConductingEquipment cimConductingEq = cimConductingEqPair.Value as FTN.ConductingEquipment;

					ResourceDescription rd = CreateConductingEqResourceDescription(cimConductingEq);
					if (rd != null)
					{
						delta.AddDeltaOperation(DeltaOpType.Insert, rd, true);
						report.Report.Append("ConudictingEq ID = ").Append(cimConductingEq.ID).Append(" SUCCESSFULLY converted to GID = ").AppendLine(rd.Id.ToString());
					}
					else
					{
						report.Report.Append("ConudictingEq ID = ").Append(cimConductingEq.ID).AppendLine(" FAILED to be converted");
					}
				}
				report.Report.AppendLine();
			}
		...
		private ResourceDescription CreateConductingEqResourceDescription(FTN.ConductingEquipment cimConductingEq)
		{
			ResourceDescription rd = null;
			if (cimConductingEq != null)
			{
				...
				rd = new ResourceDescription(gid);
				importHelper.DefineIDMapping(cimConductingEq.ID, gid);

				////populate ResourceDescription
				PowerTransformerConverter.PopulateConductingEquipmentProperties(cimConductingEq, rd, importHelper, report);
			}
			return rd;
		}
		...
		private void ImportSeason()
		{
			...
			if (cimSeasons != null)
			{
				foreach (KeyValuePair<string, object> cimSeasonPair in cimSeasons)
				{
					...
					{
						delta.AddDeltaOperation(DeltaOpType.Insert, rd, true);
						report.Report.Append("ConudictingEq ID = ").Append(cimSeason.ID).Append(" SUCCESSFULLY converted to GID = ").AppendLine(rd.Id.ToString());
					}
					else
					{
						report.Report.Append("ConudictingEq ID = ").Append(cimSeason.ID).AppendLine(" FAILED to be converted");
					}
				}
				report.Report.AppendLine();
			}
		...
		private void ImportRegulationShedule()
		{
			...
			{
				foreach (KeyValuePair<string, object> cimRegulationShedulePair in cimRegulationSchedules)
				{
					FTN.RegulationSchedule cimRegulationSchedule = cimRegulationShedulePair.Value as FTN.RegulationSchedule;
		...
		private void ImportTapSchedule()
		{
			...
			{
				foreach (KeyValuePair<string, object> cimTapScedulePair in cimTapSchedules)
				{
					FTN.TapSchedule cimTapSchedule = cimTapScedulePair.Value as FTN.TapSchedule;
		...
		private ResourceDescription CreateTapScheduleResourceDescription(FTN.TapSchedule cimTapSchedule)
		{
			...
			if (cimTapSchedule != null)
			{
				...
				////populate ResourceDescription
				PowerTransformerConverter.PopulateTapShedulesProperties(cimTapSchedule, rd, importHelper, report);
			}
			return rd;
		}
