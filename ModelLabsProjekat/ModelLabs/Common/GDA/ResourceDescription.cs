...
namespace FTN.Common
{
	...
	public class ResourceDescription
	{
		...
		public override string ToString()
		{
			...
			sb.Append("Global ID: ");
			sb.Append(String.Format("0x{0:x16}\n", this.Id));
			sb.Append("DMS Type: ");
			sb.AppendLine(((DMSType)ModelCodeHelper.ExtractTypeFromGlobalId(this.Id)).ToString());
			...
			for (int i = 0; i < this.Properties.Count; i++)
			{
				sb.AppendLine("\nProperty");
				sb.Append("Id: ");
				sb.AppendLine(this.Properties[i].Id.ToString());
				sb.Append("Value: ");
				switch (Properties[i].Type)
				{
					case PropertyType.Float:
						sb.AppendLine(this.Properties[i].AsFloat().ToString());
						break;
					...
					case PropertyType.DateTime:
						...
						{
							sb.AppendLine(String.Format("0x{0:x16}", this.Properties[i].AsLong()));
						}
						else
						{
							sb.AppendLine(this.Properties[i].AsLong().ToString());
						}
					...
					case PropertyType.Enum:
						try
						{
							...
							sb.AppendLine(enumDescs.GetStringFromEnum(this.Properties[i].Id, this.Properties[i].AsEnum()));
						}
						catch (Exception)
						{
							sb.AppendLine(this.Properties[i].AsEnum().ToString());
						}

						break;
					case PropertyType.Reference:
						sb.AppendLine(String.Format("0x{0:x16}", this.Properties[i].AsReference()));
						break;
					case PropertyType.String:
						...
						sb.AppendLine(this.Properties[i].AsString());
						break;
					...
					case PropertyType.ReferenceVector:
						if (this.Properties[i].AsLongs().Count > 0)
						{
							...
							{
								sb1.Append(String.Format("0x{0:x16}", this.Properties[i].AsLongs()[j])).Append(", ");
							}

							sb.AppendLine(sb1.ToString(0, sb1.Length - 2));
						}
						else
						{
					...
					case PropertyType.TimeSpanVector:
						if (this.Properties[i].AsLongs().Count > 0)
						{
							...
							for (int j = 0; j < this.Properties[i].AsLongs().Count; j++)
							{
								sb1.Append(String.Format("0x{0:x16}", this.Properties[i].AsTimeSpans()[j])).Append(", ");
							}
					...
					case PropertyType.Int32Vector:
						if (this.Properties[i].AsInts().Count > 0)
						{
							...
							{
								sb1.Append(String.Format("{0}", this.Properties[i].AsInts()[j])).Append(", ");
							}
					...
					case PropertyType.DateTimeVector:
						if (this.Properties[i].AsDateTimes().Count > 0)
						{
							...
							{
								sb1.Append(String.Format("{0}", this.Properties[i].AsDateTimes()[j])).Append(", ");
							}
					...
					case PropertyType.BoolVector:
						if (this.Properties[i].AsBools().Count > 0)
						{
							...
							{
								sb1.Append(String.Format("{0}", this.Properties[i].AsBools()[j])).Append(", ");
							}
					...
					case PropertyType.FloatVector:
						if (this.Properties[i].AsFloats().Count > 0)
						{
							...
							{
								sb1.Append(this.Properties[i].AsFloats()[j]).Append(", ");
							}
					...
					case PropertyType.StringVector:
						if (this.Properties[i].AsStrings().Count > 0)
						{
							...
							{
								sb1.Append(this.Properties[i].AsStrings()[j]).Append(", ");
							}
					...
					case PropertyType.EnumVector:
						if (this.Properties[i].AsEnums().Count > 0)
						{
							...
							for (int j = 0; j < this.Properties[i].AsEnums().Count; j++)
							{
								try
								{
									sb1.Append(String.Format("{0}", enumDescs.GetStringFromEnum(this.Properties[i].Id, this.Properties[i].AsEnums()[j]))).Append(", ");
								}
								catch (Exception)
								{
									sb1.Append(String.Format("{0}", this.Properties[i].AsEnums()[j])).Append(", ");
								}
							}
